...
public class GridPanel extends JPanel implements Scrollable,
    ...
    private static final Font TEXT_FONT = new Font("Verdana", Font.PLAIN, 9);
    ...
    private void drawColoredLocations(Graphics2D g2) {
        ArrayList<Location> coloredLocs = grid.getColoredLocations();
        for (Location loc : coloredLocs) {
            int xleft = colToXCoord(loc.getCol());
            int ytop = rowToYCoord(loc.getRow());
            g2.setColor(grid.getColor(loc));
            g2.fillRect(xleft, ytop, cellSize, cellSize);
        }
    }

    private void drawStrings(Graphics2D g2) {
        // System.out.println("Do nothing with drawStrings for now!!!");
    }
    ...
    private String getToolTipText(Location loc) {
        ...
        if (f != null)
            return MessageFormat.format(
                    resources.getString("cell.tooltip.nonempty"), new Object[] {
                            loc, f });
        else
            return MessageFormat.format(
                    resources.getString("cell.tooltip.empty"), new Object[] {
                            loc, f });
    }
